...
class FishingPond extends FlxState
{
	...
	public static var allThePondFish: FlxTypedGroup<PondFish>;
	...
	override public function create():Void
	{
		...
		// 			Controls.init();
		// 		}		
		FlxG.sound.playMusic("assets/music/music_pond.wav", .5, true);

		FlxG.camera.flash(0x00000000,.5);

		var numberReadSpeed = .8;

		FlxG.camera.bgColor = 0xFFf8d4d1;
		...
		var bg_pond:FlxSprite = new FlxSprite(0,0);
		bg_pond.loadGraphic("assets/images/bg_pond.png",true,320,180);
		bg_pond.animation.add("pondanimation",[0,1,2,1],6, true);
		bg_pond.animation.play("pondanimation");
		add (bg_pond);
		...
		newRecordText.setFormat("assets/fonts/BMdelico.ttf", 32, 0xFFad5a78, "left");
		newRecordText.text = "New Record";
		add(newRecordText);

		fishDesc = new FlxText(5, 5, 315);
		//fishDesc = new FlxText(0,0,200);
		fishDesc.setFormat("assets/fonts/BMdelico.ttf", 16, 0xFFad5a78, "left");
		//fishDesc.setBorderStyle(OUTLINE, 0x33FFFFFF);
		fishDesc.text = "ACTION - cast/reel";
		add (fishDesc);

		var bg:FlxSprite = new FlxSprite(0,0);
		bg.loadGraphic("assets/images/bg_pondplants.png",false,320,180);
		add (bg);
		...
		allThePondFish = new FlxTypedGroup<PondFish>();
		add (allThePondFish);
	...
	override public function update(elapsed:Float):Void
	{
		...
		//////    CASTING ROD    /////
		    ...
		    { 	
		    	...
		    	FlxTween.quadMotion(bobber, 248, 111, 199, 12, bobberLandingX, 120, .5, true, { ease: FlxEase.quadIn, type: FlxTweenType.ONESHOT, onComplete: fishingTime});
		   	}
		...
		{
			vo_Bubble.stop();
			FlxG.sound.play("assets/sounds/swoosh2.wav",.55);
			FlxG.sound.play("assets/sounds/reelin_fast.wav",.85);
			...
			allThePondFish.add(new PondFish(bobber.x,bobber.y));
			

			FlxTween.quadMotion(bobber, bobber.x, bobber.y, 199, 12, 198, 79, .5, true, { type: FlxTweenType.ONESHOT });
			FlxTween.quadMotion(allThePondFish.members[fishCaughtNumber], allThePondFish.members[fishCaughtNumber].x, allThePondFish.members[fishCaughtNumber].y, 
				200, -20, 291 - allThePondFish.members[fishCaughtNumber].width/2 + FlxG.random.int(-20, 15), 111 - allThePondFish.members[fishCaughtNumber].height/2 - fishCaughtNumber*1.5, 
	...
	public function fishCaughtNotification(Tween:FlxTween)
	{
		...
		new FlxTimer().start(1).onComplete = function(t:FlxTimer):Void
			{
				if (allThePondFish.members[fishCaughtNumber - 1].fishTypeNumber == 8) // this is to compensate for vowels in a very lazy way
					{
					...
					new FlxTimer().start(.5).onComplete = function(t:FlxTimer):Void
						{
							vo_FishName = FlxG.sound.load("assets/sounds/VO/fishing/pond/" + Std.string(allThePondFish.members[fishCaughtNumber - 1].fishTypeNumber) + ".wav",
									.7, false, null, false, false, null, playVoiceoverFishWeight.bind());
							vo_FishName.play(true);

							//FlxG.log.add("Playing: " + allThePondFish.members[fishCaughtNumber - 1].fishTypeNumber + ".wav");
							//FlxG.sound.play("assets/sounds/VO/fishing/pond/" + Std.string(allThePondFish.members[fishCaughtNumber - 1].fishTypeNumber) + ".wav",.7);
						}	
			}	
		...
		fishDesc.alpha = 1;
		bobbingAnimation.cancel();
		if (allThePondFish.members[fishCaughtNumber].fishWeight > fishRecordWeight)
		{
			FlxG.log.add("new record weight");
			newRecordText.alpha = 1;
			fishRecordWeight = allThePondFish.members[fishCaughtNumber].fishWeight;
		}

		FlxTween.tween(allThePondFish.members[fishCaughtNumber], {y: allThePondFish.members[fishCaughtNumber].y - 3}, .25, {ease: FlxEase.bounceOut, type: FlxTweenType.PINGPONG, loopDelay: 500});
		FlxG.log.add("caught fish #" + fishCaughtNumber + "-- " + allThePondFish.members[fishCaughtNumber].fishName);

		var roundedWeight:Float = FlxMath.roundDecimal(allThePondFish.members[fishCaughtNumber].fishWeight,2);

		if (allThePondFish.members[fishCaughtNumber].fishWeight / 1000 >= 1.0)
			{
				// fishDesc.text = allThePondFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(allThePondFish.members[fishCaughtNumber].fishWeight / 1000, true, true) + "kg" ;
				fishDesc.text = allThePondFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(roundedWeight / 1000, true, true) + "kg" ;
			}

		else if (allThePondFish.members[fishCaughtNumber].fishWeight / 1000 < 1.0)
			{
				fishDesc.text = allThePondFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(roundedWeight, true, true) + "g" ;
			}
	...
	{
		FlxG.sound.play("assets/sounds/bloop.wav",.5);
		FlxG.log.add("bobber hitting water");
			sploosh.x = bobber.x-10;
			sploosh.y = bobber.y-10;
			sploosh.animation.play("sploosh");
	...
	{
		var _fishWeight:Float = allThePondFish.members[fishCaughtNumber - 1].fishWeight;
