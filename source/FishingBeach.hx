...
class FishingBeach extends FlxState
{
	...
	public static var allTheBeachFish: FlxTypedGroup<BeachFish>;
	...
	override public function create():Void
	{
		...
		FlxG.sound.playMusic("assets/music/music_beach.wav", .75, true);
		FlxG.camera.flash(0x00000000,.5);

		var numberReadSpeed = .8;

		var bg:FlxSprite = new FlxSprite(0,0);
		bg.loadGraphic("assets/images/bg_beach.png",true,320,180);
		bg.animation.add("bgAnimation",[0,1,2,1], 3, true);
		bg.animation.play("bgAnimation");
		add (bg);
		...
		allTheBeachFish = new FlxTypedGroup<BeachFish>();
		add (allTheBeachFish);
		...
		newRecordText.setFormat("assets/fonts/BMdelico.ttf", 32, 0xFF143262, "left");
		newRecordText.text = "New Record";
		add(newRecordText);

		fishDesc = new FlxText(5, 5, 315);
		//fishDesc = new FlxText(0,0,200);
		fishDesc.setFormat("assets/fonts/BMdelico.ttf", 16, 0xFF143262, "left");
	...
	override public function update(elapsed:Float):Void
	{
		if (FlxG.keys.anyJustPressed([ESCAPE, A]))
		{
				...
				// when the transition is done, switches to Fishingocean
		...
		if (FlxG.keys.anyJustPressed([SPACE, B]) && isCasting == false && isFishing == false && controlsDisabled == false)
		    { 	
		    	FlxG.sound.play("assets/sounds/swish1.wav",1);
		...
		//////    CASTING ROD    /////
		    ...
		    { 	
		    	FlxG.sound.play("assets/sounds/swoosh1.wav",1);
		    	FlxG.sound.play("assets/sounds/reelout_fast.wav",1);
		    	...
		    	FlxTween.quadMotion(bobber, 248, 111, 199, 12, bobberLandingX, 112, .5, true, { ease: FlxEase.quadIn, type: FlxTweenType.ONESHOT, onComplete: fishingTime});
		   	}
		...
		{
			vo_Bubble.stop();
			FlxG.sound.play("assets/sounds/swoosh2.wav",1);
			FlxG.sound.play("assets/sounds/reelin_fast.wav",1);
			...
			allTheBeachFish.add(new BeachFish(bobber.x,bobber.y));
			

			FlxTween.quadMotion(bobber, bobber.x, bobber.y, 199, 12, 198, 79, .5, true, { type: FlxTweenType.ONESHOT });
			FlxTween.quadMotion(allTheBeachFish.members[fishCaughtNumber], allTheBeachFish.members[fishCaughtNumber].x, allTheBeachFish.members[fishCaughtNumber].y, 
				200, -20, 291 - allTheBeachFish.members[fishCaughtNumber].width/2 + FlxG.random.int(-20, 15), 111 - allTheBeachFish.members[fishCaughtNumber].height/2 - fishCaughtNumber*1.5, 
	...
	{
		FlxG.sound.play("assets/sounds/pop1.wav",1);
		new FlxTimer().start(.1).onComplete = function(t:FlxTimer):Void
			{
				FlxG.sound.play("assets/sounds/arpeggio2-good.wav",.55);
			}

		new FlxTimer().start(1).onComplete = function(t:FlxTimer):Void
			{
				if (allTheBeachFish.members[fishCaughtNumber - 1].fishTypeNumber == 4) // this is to compensate for vowels in a very lazy way
					{
						vo_YouCaughtAn.play(true);
						//FlxG.sound.play("assets/sounds/VO/fishing/youcaughtan.wav",.7);
					}

				else {
					vo_YouCaughtA.play(true);
					//FlxG.sound.play("assets/sounds/VO/fishing/youcaughta.wav",.7);
				}


					new FlxTimer().start(.5).onComplete = function(t:FlxTimer):Void
						{
							vo_FishName = FlxG.sound.load("assets/sounds/VO/fishing/ocean/" + allTheBeachFish.members[fishCaughtNumber - 1].fishShortName + ".wav",
									.7, false, null, false, false, null, playVoiceoverFishWeight.bind());
							vo_FishName.play(true);

							//FlxG.log.add("Playing: " + allTheBeachFish.members[fishCaughtNumber - 1].fishTypeNumber + ".wav");
							//FlxG.sound.play("assets/sounds/VO/fishing/ocean/" + Std.string(allTheBeachFish.members[fishCaughtNumber - 1].fishTypeNumber) + ".wav",.7);
						}	
			}	
		...
		fishDesc.alpha = 1;
		bobbingAnimation.cancel();
		if (allTheBeachFish.members[fishCaughtNumber].fishWeight > fishRecordWeight)
		{
			FlxG.log.add("new record weight");
			newRecordText.alpha = 1;
			fishRecordWeight = allTheBeachFish.members[fishCaughtNumber].fishWeight;
		}

		FlxTween.tween(allTheBeachFish.members[fishCaughtNumber], {y: allTheBeachFish.members[fishCaughtNumber].y - 3}, .25, {ease: FlxEase.bounceOut, type: FlxTweenType.PINGPONG, loopDelay: 500});
		FlxG.log.add("caught fish #" + fishCaughtNumber + "-- " + allTheBeachFish.members[fishCaughtNumber].fishName);

		var roundedWeight:Float = FlxMath.roundDecimal(allTheBeachFish.members[fishCaughtNumber].fishWeight,2);

		if (allTheBeachFish.members[fishCaughtNumber].fishWeight / 1000 >= 1.0)
			{
				// fishDesc.text = allTheBeachFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(allTheBeachFish.members[fishCaughtNumber].fishWeight / 1000, true, true) + "kg" ;
				fishDesc.text = allTheBeachFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(roundedWeight / 1000, true, true) + "kg" ;
			}

		else if (allTheBeachFish.members[fishCaughtNumber].fishWeight / 1000 < 1.0)
			{
				fishDesc.text = allTheBeachFish.members[fishCaughtNumber].fishName + " - " + FlxStringUtil.formatMoney(roundedWeight, true, true) + "g" ;
			}
	...
	{
		FlxG.sound.play("assets/sounds/bloop.wav",.75);
		FlxG.log.add("bobber hitting water");
			sploosh.x = bobber.x-10;
			sploosh.y = bobber.y-10;
			sploosh.animation.play("sploosh");
	...
	private function fishBiting(Timer:FlxTimer)
	{
		vo_Bubble.play(true);
		//FlxG.sound.play("assets/sounds/bubbles.wav",.25);
		FlxG.sound.play("assets/sounds/arpeggio-ohno.wav",.45);
		bobbingAnimation.cancel();
	...
	{
		var _fishWeight:Float = allTheBeachFish.members[fishCaughtNumber - 1].fishWeight;
